...
use super::{
    types::{AnyType, MapType, Model, ModelType},
...
pub fn from_object_with_additional_properties(
    ...
    match schema.get("additionalProperties") {
        Some(value) => match value {
            ...
            Value::Bool(true) => Ok(Model::new(ModelType::MapType(MapType {
                name: Some(name),
                model: Box::new(AnyType::model(schema, scope).flatten(container, scope)?),
            }))),
...
mod tests {
    ...
    #[test]
    fn test_should_convert_to_map_on_additional_properties_true() {
